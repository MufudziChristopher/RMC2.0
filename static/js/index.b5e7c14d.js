! function() {
	...
	class l {
		...
		fx1() {
			...
			this.clearCells();
			const e = (o, s, i = 0) => {
				s.cache = s.state, 44 === i ? (s.set(s.original), ++t, t === this.totalChars && (this.isAnimating = !1)) : 0 === s.position ? s.set(i < 9 ? ["*", "-", "'", '"'][Math.floor(4 * Math.random())] : this.getRandomChar()) : s.set(o.cells[s.previousCellPosition].cache), "&nbsp;" != s.cache && ++i, i < 45 && setTimeout((() => e(o, s, i)), 5)
			};
			for(const t of this.lines)
				for(const o of t.cells) setTimeout((() => e(t, o)), 20 * (t.position + 1))
		}
		fx2() {
			let t = 0;
			const e = (o, s, i = 0) => {
				19 === i ? (s.set(s.original), s.DOM.el.style.opacity = 0, setTimeout((() => {
					s.DOM.el.style.opacity = 1
				}), 300), ++t, t === this.totalChars && (this.isAnimating = !1)) : s.set(this.getRandomChar()), ++i < 20 && setTimeout((() => e(o, s, i)), 40)
			};
			for(const t of this.lines)
		...
		fx3() {
			...
			this.clearCells();
			const o = (e, s, i = 0) => {
				9 === i ? (s.set(s.original), ++t, t === this.totalChars && (this.isAnimating = !1)) : s.set(this.getRandomChar()), ++i < 10 && setTimeout((() => o(e, s, i)), 80)
			};
			for(const t of this.lines)
		...
		fx4() {
			...
			this.clearCells();
			const e = (o, s, i = 0) => {
				s.cache = s.state, 29 === i ? (s.set(s.original), ++t, t === this.totalChars && (this.isAnimating = !1)) : 0 === s.position ? s.set(["*", ":"][Math.floor(2 * Math.random())]) : s.set(o.cells[s.previousCellPosition].cache), "&nbsp;" != s.cache && ++i, i < 30 && setTimeout((() => e(o, s, i)), 15)
			};
			for(const t of this.lines)
				for(const o of t.cells) setTimeout((() => e(t, o)), 400 * Math.abs(this.lines.length / 2 - t.position))
		}
		fx5() {
			let t = 0;
			this.clearCells();
			const e = (o, s, i = 0) => {
				s.cache = {
					state: s.state,
					color: s.color
				}, 29 === i ? (s.color = s.originalColor, s.DOM.el.style.color = s.color, s.set(s.original), ++t, t === this.totalChars && (this.isAnimating = !1)) : 0 === s.position ? (s.color = ["#3e775d", "#61dca3", "#61b3dc"][Math.floor(3 * Math.random())], s.DOM.el.style.color = s.color, s.set(i < 9 ? ["*", "-", "'", '"'][Math.floor(4 * Math.random())] : this.getRandomChar())) : (s.set(o.cells[s.previousCellPosition].cache.state), s.color = o.cells[s.previousCellPosition].cache.color, s.DOM.el.style.color = s.color), "&nbsp;" != s.cache.state && ++i, i < 30 && setTimeout((() => e(o, s, i)), 10)
			};
			for(const t of this.lines)
				for(const o of t.cells) setTimeout((() => e(t, o)), 200 * (t.position + 1))
		}
		fx6() {
			let t = 0;
			const o = (s, i, n = 0) => {
				i.cache = {
					state: i.state,
					color: i.color
				}, 14 === n ? (i.set(i.original), i.color = i.originalColor, i.DOM.el.style.color = i.color, ++t, t === this.totalChars && (this.isAnimating = !1)) : (i.set(this.getRandomChar()), i.color = ["#2b4539", "#61dca3", "#61b3dc"][Math.floor(3 * Math.random())], i.DOM.el.style.color = i.color), ++n < 15 && setTimeout((() => o(s, i, n)), e(30, 110))
			};
			for(const t of this.lines)
		...
		constructor(e) {
			o(this, "DOM", {
				el: null
			}), o(this, "lines", []), o(this, "lettersAndSymbols", ["A", "9"]), o(this, "effects", {
				fx1: () => this.fx1()
				// fx2: () => this.fx2(),
				// fx3: () => this.fx3(),
				// fx4: () => this.fx4(),
